...
//
// Keyed by contactId rather than facilityCode/districtCode because the
// contact is the stable identity. If contact data ever changes mid-session
// (not currently possible but defensive), a different contactId would
// produce a fresh fetch automatically.
// ─────────────────────────────────────────────────────────────────────────────
const membershipLinksCache = new Map<string, MembershipLinksResponse>();

// ─────────────────────────────────────────────────────────────────────────────
// HOOK STATE SHAPE
// Three mutually exclusive states matching useResources pattern.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// useMembershipLinks
// Fetches membership links for the currently authenticated contact.
// Caches the result by contactId for the lifetime of the browser session.
//
...
export function useMembershipLinks(): UseMembershipLinksState {
    ...
    useEffect(() => {
        ...
        // ─────────────────────────────────────────────────────────────────────
        // ABORT CONTROLLER
        // If the component unmounts before the fetch completes (e.g. user
        // navigates away), the request is aborted and the result discarded.
